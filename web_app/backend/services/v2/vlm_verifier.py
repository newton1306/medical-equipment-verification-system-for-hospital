# -*- coding: utf-8 -*-
"""Gemini VLM verification — direct full-image + CoT strategy for Version 2."""
...
def call_vlm(
    ...
    """Send direct full-resolution image to Gemini for V2 verification. Auto-retries on 429."""
    ...
    parts.append('Full tray overview:')
    parts.append(_bgr_to_pil(full_image))
...
def verify_with_vlm(
    ...
    """Tiered verification for V2: Flash first, escalate to Pro if uncertain."""
    result = call_vlm(full_image, reference_img, checklist, GEMINI_MODEL_FAST)

    if (result.get('status') == 'UNCERTAIN'
            or result.get('confidence', 0) < GEMINI_ESCALATION_THRESHOLD):
        result = call_vlm(full_image, reference_img, checklist, GEMINI_MODEL_PRO)
